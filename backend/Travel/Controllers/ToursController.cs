...
namespace Travel.Controllers
{
    ...
    public class ToursController : Controller
    {
        ...
        public async Task<IActionResult> confirm_dahoantien([FromRoute] int id)
        {
            try
            {
                HoaDon hoaDon = _context.HoaDons.Include(h => h.NguoiDung).Include(t => t.Tour).Where(t => (t.TrangThai == 5 || t.TrangThai == 8 || t.TrangThai == 10) && t.Id == id).FirstOrDefault();
                if (hoaDon == null)
                {
                    return StatusCode(404, "Hóa đơn không hợp lệ");
                }
                if (hoaDon.TrangThai == 5 || hoaDon.TrangThai == 10)
                {
                    hoaDon.TrangThai = 9;
                }
                else
        ...
        public async Task<IActionResult> get_tour_by_diemdenid([FromRoute] int id)
        {
            try
            {
                ...
                List<Tour> tours = _context.Tours.Where(t => t.TrangThai == 1 && t.DiemDen == id).ToList();
                ...
                foreach (var tour in tours)
                {
                    ...
                    tour_Serialize.Tentour = tour.TenTour;
                    tour_Serialize.Theloai = tour.TheLoaiId;
                    tour_Serialize.Tentheloai = _context.TheLoais.Where(d => d.Id == tour.TheLoaiId).FirstOrDefault().TenLoai;
                    tour_Serialize.Phanvung = tour.PhanVungId;
                    tour_Serialize.Tenphanvung = _context.PhanVungs.Where(d => d.Id == tour.PhanVungId).FirstOrDefault().TenVung;
                    tour_Serialize.Congty = tour.CongTyId;
                    tour_Serialize.Tencongty = _context.CongTies.Where(d => d.Id == tour.CongTyId).FirstOrDefault().Tencongty;
                    tour_Serialize.VeToiDa = tour.VeDoiDa;
                    tour_Serialize.VeToiThieu = tour.VeToiThieu;
                    tour_Serialize.SoNgay = tour.SoNgay;
                    tour_Serialize.SoDem = tour.SoDem;
        ...
        public async Task<IActionResult> search_tour([FromBody] search_serialize search_Serialize)
        {
            try
            {
                ...
                List<Tour_serialize> result = new List<Tour_serialize>();
                List<Tour> tours = _context.Tours.Include(t => t.TheLoai).Include(t => t.CongTy).Include(t => t.PhanVung).Where(t => t.TrangThai == 1).OrderByDescending(t => t.NgayTao).ToList();
                List<Tour> tours_cp = _context.Tours.Include(t => t.TheLoai).Include(t => t.CongTy).Include(t => t.PhanVung).Where(t => t.TrangThai == 1).OrderByDescending(t => t.NgayTao).ToList();
                ...
                {
                    for (int i = 0; i<tours_cp.Count(); i++)
                    {
                        int id = tours_cp[i].Id;
                        List<DiaDiem_Tour> diaDiem_Tours = _context.DiaDiem_Tours.Where(d =>d.TourId == tours_cp[i].Id && d.DiaDiemId == Convert.ToInt32(search_Serialize.diemden)).ToList();
                        Tour obj_tour = _context.Tours.Where(t => t.TrangThai == 1 && t.Id == tours_cp[i].Id && t.DiemDen == Convert.ToInt32(search_Serialize.diemden)).FirstOrDefault();
                        if (obj_tour == null && diaDiem_Tours.Count() < 1)
                        {
                            tours.Remove(tours_cp[i]);
